...
class Telephone {
  ...
  DialPhoneNumber(phoneNumber) {
    if (!this.phoneNumbers.includes(phoneNumber)) return;
...
rl.on("line", (line) => {
  ...
  switch (command) {
    case "add":
      if (!arg) console.log("Usage: add <number>");
      else {
    ...
    case "remove":
      if (!arg) console.log("Usage: remove <number>");
      else {
    ...
    case "dial":
      if (!arg) console.log("Usage: dial <number>");
      else if (!telephone.phoneNumbers.includes(arg)) {
    ...
    case "list":
      if (telephone.phoneNumbers.length === 0) console.log("(no numbers saved)");
      else telephone.phoneNumbers.forEach((n) => console.log(`  ${n}`));
      break;
